...
from baseline.solution.implementation.solution import solve
...
class SolutionDemo(SolutionAbstract):
    ...
    def get_solution(self):
        input_file = 'files/' + self.epicrisis.path_to_xml
        search_text = 'АНАМНЕЗ'
        
        # tree = etree.parse(input_file)  
        sol = solve(input_file, search_text)
        
        output_dir = 'files/' + f'sessions/{self.epicrisis.session_id}/output/'
        output_file = output_dir + f'{self.epicrisis.epicrisis_id}_{self.epicrisis.version_id}' \
                                          f'_{self.epicrisis.task_id}.json'
        
        #Не обязательно создавать файл
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        with open(output_file, 'w') as f:
            json.dump(sol, f, indent=4, ensure_ascii=False)
